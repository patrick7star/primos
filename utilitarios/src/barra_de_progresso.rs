...
impl ProgressoTemporal {
   // criando uma instância...
   pub fn cria(qtd_total:u64, ritmo:u16) -> Self {
      // criando dado agora...
      ProgressoTemporal {
         qtd_atual: 0,  
         qtd_total,
         atual: Duration::new(0, 0),
         total: Duration::from_secs(qtd_total),
         cronometro: Instant::now(),
         ritmo: Duration::from_millis(ritmo as u64),
      }
   }
   ...
   pub fn imprime(&mut self) -> Option<String> {
      ...
      if  tp >= self.ritmo  {
         ...
         let bp = progresso(self.qtd_atual, self.qtd_total);
         // atualizando variação ocorrida do tempo.
         self.atual = self.atual.checked_add(self.ritmo).unwrap();
   ...
   pub fn percentual(&self) -> f32 
      { self.atual.as_secs_f32() / self.total.as_secs_f32() }
